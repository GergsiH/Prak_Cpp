...
class Lexeme 
{
    private:
        ...
        std::std::string str;
    public:
        Lexeme(typelex t = LEX_NULL, int v = 0, double d = 0.0, std::std::string s = "")
        {
            type = t;
            value = v;
            str = s;
        ...
        void putString(std::std::string s)
        {
            str = s;
        }
...
class Identifier
{
    ...
    public:
        ...
        Identifier(const std::std::string n)
        {
            name = n;
            declare = false;
            assign = false;
        ...
        bool operator==(const std::std::string& s) const;
};

bool Identifier::operator==(const std::std::string& s) const 
{
    return name == s;
}
...
int put (const std::std::string & buf) 
{
...
class Scanner
{
    private:
        FILE *f;
        char c;
        int in (const std::std::string& buf, const char ** list) 
        {
            int i = 0;
            while (list[i]) 
            {
...
const char* Scanner::words [] = {"", "program", "int", "std::string", "real", "true", "false","if", "else", "for", "while", "read", "write", "break", "not", "or", "and", NULL};
